...
import React, { useContext } from 'react'
 ...
 const CartPage = () => {
    ...
    const increaseQty = (cartItem: any) => {
      ...
      addToCart(item); // Correct function for increasing quantity
    };
    
    const decreaseQty = (cartItem: any) => {
      if (cartItem?.quantity <= 1) {
        clearCart(cartItem); // If quantity reaches 0, remove the item entirely
        return;
      }
    
      removeFromCart(cartItem); // Correct function for decreasing quantity
    };
    ...
    if (cartItems.length === 0) return(<div className="w-full h-[100vh] flex items-center justify-center">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <aside className="md:w-1/4">
                <article className="border border-gray-200 bg-white shadow-sm rounded mb-5 p-3 lg:p-5">
                  ...
                  <Link href="/payment" className="px-4 py-3 mb-2 inline-block text-lg w-full text-center font-medium text-white bg-green-600 border border-transparent rounded-md hover:bg-green-700 cursor-pointer">
                  Proceed to Payment
                  </Link>
